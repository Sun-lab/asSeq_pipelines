...
# Outline
# 1) Get GWAS catalog file
# 2) Input data.frame of eQTL data
# 3) Calculate jackknife and all-data enrichment estimates
# 4) Plot
...
# Minor functions
# ----------
ST_name_change = function(DATA,ORIG_NAME,NEW_NAME){
	...
	if( length(new_idx) > 0 ){
		...
		stop("ORIG_NAME missing!")
	}
	
}
ST_smart_rmcols = function(OBJ,rm_names){
...
ST_smart_df = function(...){
	data.frame(...,stringsAsFactors = FALSE)
}
ST_calc_JK = function(EST,LOO_EST,alpha = 0.05){
	# Assume EST is a scalar or numeric vector
	# Assume LOO_EST is a numeric vector or matrix
	# If LOO_EST is matrix, length(EST) == ncol(LOO_EST)
...
get_GWAS_catalog = function(work_dir){
  ...
  fnm = "gwas_catalog_v1.0-associations_e104_r2021-11-05.tsv.gz"
  
	gwas_rds_fn = file.path(work_dir, "gwas_catalog_v1.0.rds")
	gwas_fn     = file.path(work_dir, fnm)
	
	if( !file.exists(gwas_fn) ){
	  stop(paste("cannot find the gwas catalog file:", gwas_fn))
	}
	
	if( !file.exists(gwas_rds_fn) ){

		cat(sprintf("%s: Import GWAS catalog ...\n",date()))
		gwas = suppressWarnings(data.table::fread(gwas_fn,
		...
		# Polish gwas
		gwas = ST_name_change(gwas,"DISEASE/TRAIT","PHENO")
		...
		gwas = ST_smart_rmcols(gwas,c("CNV","MERGED","CONTEXT","PLATFORM [SNPS PASSING QC]",
...
run_gwasEnrich_analysis = function(DATA,work_dir,
	...
	for(one_EE in all_EE){
		...
		tmp_df = ST_smart_df(GROUP = GROUP,
	...
	for(one_EE in all_EE){
		...
		if( gwas_enrich$n_gwas_eqtl[idx] > 0 ){
			...
			tmp_df = ST_smart_df(BLOCK = BLOCK,GROUP = GROUP,
				n_test = num_test_loci,n_eqtl = num_eqtl,
				n_gwas = num_gwas,n_gwas_eqtl = num_eqtl_gwas)
		} else {
			tmp_df = ST_smart_df(BLOCK = BLOCK,GROUP = GROUP,
	...
	for(one_EE in all_EE){
		...
		out_JK = ST_calc_JK(EST = log(gwas_enrich$enrich[idx1]),
	...
	gwas_enrich = ST_smart_df(wGWAS = which_gwas,gwas_enrich)
...
sim_EQTL_RES = function(work_dir){ 
	...
	num_loci = 4e4
	dat = ST_smart_df(Chr = rep("chr1",num_loci))
	prob = c(1,1); prob = prob / sum(prob)
	
	dat$CLASS = sample(c(0,1),num_loci,replace = TRUE,prob = prob)
	dat$POS = NA
	idx = which(dat$CLASS == 0); length(idx)
	max_pos = max(as.integer(gwas$CHR_POS))
	
	dat$POS[idx] = sample(max_pos, length(idx), replace = FALSE)
	idx = which(dat$CLASS == 1)
	dat$POS[idx] = sample(as.integer(gwas$CHR_POS), length(idx), replace = FALSE)
	dat[1:10,]
	
	prob = c(5,1); prob = prob / sum(prob)
	dat$EE_A1 = sample(c(0,1),num_loci, replace = TRUE,prob = prob)
	prob = c(5,1); prob = prob / sum(prob)
	dat$EE_A2 = sample(c(0,1),num_loci, replace = TRUE,prob = prob)
	
	# Induce enrichment
	cat(sprintf("%s: Induce enrichment in simulated dataset ...\n", date()))
	idx = which(dat$CLASS == 1)
	
	dat$EE_A2[idx] = sapply(dat$EE_A2[idx], 
	                        function(xx){ ifelse(xx == 1, 1, rbinom(1,1,0.1))},
	                        USE.NAMES = FALSE)
	...
	for(wGWAS in names(tab)){
	  ...
	  res1 = run_gwasEnrich_analysis(DATA = dat,
	                                 work_dir = work_dir,
	                                 which_gwas = wGWAS,
	                                 nBLOCKS = 200,
	                                 verbose = FALSE)
